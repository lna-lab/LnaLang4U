...
class HiRadixCache(RadixCache):
    ...
    def __init__(self, params: CacheInitParams, server_args: ServerArgs):
        ...
        if isinstance(self.kv_cache, DeepSeekV4TokenToKVPool):
            if server_args.hicache_io_backend != "direct":
                raise ValueError(
                    "DeepSeek-V4 HiCache requires direct IO. Please launch with "
                    "--hicache-io-backend direct --hicache-mem-layout page_first."
                )
            from sglang.srt.mem_cache.deepseek_v4_memory_pool_host import (
            ...
            raise ValueError(
                "HiRadixCache only supports MHA, MLA, and DeepSeek-V4 yet"
            )
    ...
    def supports_swa(self) -> bool:
        return (
            ...
            and getattr(self.kv_cache, "swa_kv_pool", None) is not None
        )
    ...
    def dec_lock_ref(self, node, swa_uuid=None):
        return super().dec_lock_ref(node)
